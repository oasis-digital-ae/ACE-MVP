...
export const BuyWindowIndicator: React.FC<BuyWindowIndicatorProps> = ({ 
  ...
  return (
    <div className={`flex flex-col ${compact ? 'gap-0.5' : 'gap-1'} ${textSize}`}>
      {status.isOpen ? (
        // When open: just show icon and time
        <>
          <div className="flex items-center gap-1.5">
            ...
            {showCountdown && timeLeft && (
              <span className="text-green-400/80">{timeLeft}</span>
            )}
          </div>
        </>
      ) : (
        // When closed: show icon, "Closed" text, and time
        <>
          <div className="flex items-center gap-1.5">
          ...
          {showCountdown && timeLeft && (
            <div className="pl-[18px] text-red-400/80">
              {timeLeft}
            </div>
          )}
        </>
