...
export const CreditWalletPanel: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to load credit wallet data',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
  ...
  const handleLoanSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to process loan',
        variant: 'destructive'
      });
    } finally {
      setSubmitting(false);
  ...
  const handleReverseLoan = async (transactionId: number, username: string, amountVal: number) => {
    if (!confirm(`Are you sure you want to reverse this credit loan of ${formatCurrency(amountVal)} to ${username}? This will deduct the amount from their wallet.`)) {
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `Reversed credit loan of ${formatCurrency(amountVal)} to ${username}`
      });
      await loadData();
    } catch (error: unknown) {
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to reverse credit loan',
        variant: 'destructive'
      });
    } finally {
      setReversing(null);
                  ...
                  {summary.perUserBreakdown.map((row) => (
                    <tr key={row.user_id} className="border-b hover:bg-muted/30">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatCurrency(fromCents(row.total_credit_cents).toNumber())}
                      </td>
                    ...
                    {creditLoans.map((tx) => (
                      <tr key={tx.id} className="border-b hover:bg-muted/30">
                        ...
                        <td className="px-4 py-2 text-right font-mono text-green-600">
                          +{formatCurrency(fromCents(tx.amount_cents).toNumber())}
                        </td>
                        ...
                        <td className="px-4 py-2 text-center">
                          {tx.is_reversed ? (
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleReverseLoan(tx.id, tx.username, fromCents(tx.amount_cents).toNumber())}
