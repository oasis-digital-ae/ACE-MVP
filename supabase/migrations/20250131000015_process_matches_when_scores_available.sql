-- Process All Matches When Scores Are Available
-- NOTE: This migration will process matches that have scores set.
-- If scores are missing, you need to fetch them from the Football API first.
-- 
-- To fetch scores:
-- 1. Run the match update service/function to fetch scores from Football API
-- 2. Then this migration will process all matches with scores chronologically
...
BEGIN
  ...
  IF v_fixtures_with_scores = 0 THEN
    ...
    RAISE NOTICE 'Please fetch match scores from the Football API first, then re-run this migration.';
    RAISE NOTICE 'You can use the match update service or sync fixtures from the API.';
    RETURN;
  END IF;
