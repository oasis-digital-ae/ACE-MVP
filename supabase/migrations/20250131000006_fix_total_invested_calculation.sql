...
-- CRITICAL: total_invested must be calculated as ROUND(price, 2) * quantity
-- NOT as ROUND(price * quantity, 2)
-- This ensures avgCost = total_invested / quantity = ROUND(price, 2) exactly

-- Step 1: Fix existing positions to use correct calculation
UPDATE positions p
SET total_invested = ROUND(
  (SELECT ROUND(t.market_cap / 1000, 2) FROM teams t WHERE t.id = p.team_id) * p.quantity,
  2
)
WHERE EXISTS (
  SELECT 1 FROM teams t 
  WHERE t.id = p.team_id 
  AND ABS(p.total_invested - (ROUND(t.market_cap / 1000, 2) * p.quantity)) > 0.01
);

-- Step 2: Update purchase function to calculate total_invested correctly
...
BEGIN
  -- CRITICAL: Round price_per_share FIRST, then multiply by shares
  -- This ensures total_invested / quantity = price_per_share exactly
  v_rounded_price_per_share := ROUND(p_price_per_share, 2);
  v_correct_total_amount := ROUND(v_rounded_price_per_share * p_shares, 2);
  ...
  IF v_wallet_balance < v_correct_total_amount THEN
    PERFORM set_config('app.allow_wallet_update', '', true);
    RETURN json_build_object(
      'success', false,
      'error', format('Insufficient balance. Required: %s, Available: %s', v_correct_total_amount, v_wallet_balance)
    );
  END IF;
  ...
  IF ABS(v_rounded_price_per_share - v_nav) > 0.01 THEN
    PERFORM set_config('app.allow_wallet_update', '', true);
    RAISE EXCEPTION 'Price mismatch: expected %, got %', v_nav, v_rounded_price_per_share;
  END IF;
  ...
  IF FOUND THEN
    -- CRITICAL: Calculate new total_invested as (rounded_price * new_shares) + existing
    -- This ensures avgCost = total_invested / total_quantity = rounded_price exactly
    v_new_total_invested := ROUND(v_existing_total_invested + v_correct_total_amount, 2);
    UPDATE positions
    SET 
  ...
  INSERT INTO total_ledger (
    ...
    v_shares_outstanding_before, v_shares_outstanding_after,
    v_rounded_price_per_share, v_nav,
...
-- Step 3: Update sell function similarly
...
COMMENT ON FUNCTION process_share_purchase_atomic IS 'CRITICAL: total_invested = ROUND(price_per_share, 2) * shares. This ensures avgCost = total_invested / quantity = price_per_share exactly. No rounding discrepancies.';
