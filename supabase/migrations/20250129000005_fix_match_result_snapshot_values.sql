-- Fix match result processing to use snapshot values for market_cap_before/after
-- The transfer amount is calculated from snapshots, so the before/after values should also use snapshots
-- This ensures the displayed impact matches the actual transfer amount
...
COMMENT ON FUNCTION process_match_result_atomic IS 'Fixed shares model: Price = market_cap / total_shares (1000). Uses snapshot values for accurate historical display of match impacts.';
