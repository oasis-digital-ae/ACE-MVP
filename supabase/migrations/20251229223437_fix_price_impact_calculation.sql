...
-- The issue: price_impact is calculated from share prices that have already been rounded to 2 decimals
-- This causes discrepancies (e.g., Man City loses $0.49 but Brighton gains $0.50)
-- Fix: Calculate price_impact from 4-decimal prices, then round the result
...
BEGIN
  ...
  ELSE
    ...
    IF NOT v_home_entry_exists THEN
      ...
      v_winner_price_before := ROUND(v_winner_price_before_4dec, 2); -- Round to 2 for storage
      v_winner_price_impact := 0; -- No change for draws
  ...
  -- CRITICAL FIX: Calculate prices with 4-decimal precision FIRST, then calculate impact, then round
