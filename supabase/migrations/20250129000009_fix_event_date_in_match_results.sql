-- Fix event_date in process_match_result_atomic to use fixture kickoff_at instead of current timestamp
-- This ensures match history shows the correct dates
...
COMMENT ON FUNCTION process_match_result_atomic IS 'Fixed shares model: Price = market_cap / total_shares (1000). Uses snapshot values for accurate historical display. Sets event_date to fixture kickoff_at.';

-- Now update all existing entries to use the correct event_date from fixtures
