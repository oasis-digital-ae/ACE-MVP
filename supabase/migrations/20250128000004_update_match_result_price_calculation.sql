-- Update Match Result Processing to Use total_shares (Fixed Shares Model)
-- Price calculation: market_cap / total_shares (1000) instead of market_cap / shares_outstanding
...
DECLARE
  ...
  v_total_shares INTEGER; -- Use total_shares instead of shares_outstanding
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO total_ledger (
      ...
      v_total_shares, v_total_shares, -- Fixed total_shares
  ...
  -- Log WINNER to total_ledger (use total_shares in shares_outstanding fields for consistency)
  INSERT INTO total_ledger (
    ...
    v_winner_before_cap, v_winner_after_cap, v_total_shares, v_total_shares, -- Fixed total_shares
  ...
  -- Log LOSER to total_ledger (use total_shares in shares_outstanding fields for consistency)
  INSERT INTO total_ledger (
    team_id, ledger_type, amount_transferred, opponent_team_id, opponent_team_name, is_home_match,
    market_cap_before, market_cap_after, shares_outstanding_before, shares_outstanding_after,
    share_price_before, share_price_after, price_impact, match_result, match_score,
    ...
    v_loser_before_cap, GREATEST(v_loser_after_cap, 10), v_total_shares, v_total_shares, -- Fixed total_shares
...
COMMENT ON FUNCTION process_match_result_atomic IS 'Fixed shares model: Price = market_cap / total_shares (1000). Market cap transfers unchanged.';
