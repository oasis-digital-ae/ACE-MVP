-- Enforce Perfect Conservation After Rounding to 2 Decimals
-- The issue: We enforce conservation at 4-decimal precision, then round to 2 decimals
-- This rounding can introduce drift. Fix: Enforce conservation AFTER rounding to 2 decimals
...
DECLARE
  ...
  v_winner_new_cap_4dec NUMERIC;
  v_loser_new_cap_4dec NUMERIC;
  v_winner_new_cap_2dec NUMERIC;
  v_loser_new_cap_2dec NUMERIC;
  v_pair_total_before_4dec NUMERIC;
  v_pair_total_after_4dec NUMERIC;
  v_pair_total_before_2dec NUMERIC;
  v_pair_total_after_2dec NUMERIC;
  v_conservation_drift_2dec NUMERIC;
BEGIN
  ...
  IF v_fixture.result = 'home_win' THEN
    ...
    v_transfer_amount := ROUND(v_away_snapshot_cap * 0.10, 4);
  ...
  ELSIF v_fixture.result = 'away_win' THEN
    ...
    v_transfer_amount := ROUND(v_home_snapshot_cap * 0.10, 4);
  ...
  ELSE
    -- Draw handling (same as before, but simplified)
    v_match_score := COALESCE(v_fixture.home_score, 0) || '-' || COALESCE(v_fixture.away_score, 0);
  ...
  v_pair_total_before_4dec := ROUND(v_winner_current_cap + v_loser_current_cap, 4);
  v_pair_total_before_2dec := ROUND(v_pair_total_before_4dec, 2);
  ...
  IF v_loser_current_cap <= 10.00 THEN
    ...
    v_winner_new_cap_4dec := v_winner_current_cap;
    v_loser_new_cap_4dec := v_loser_current_cap;
  ELSE
    -- Calculate new caps with 4-decimal precision internally
    v_winner_new_cap_4dec := ROUND(v_winner_current_cap + v_transfer_amount, 4);
    v_loser_new_cap_4dec := ROUND(v_loser_current_cap - v_transfer_amount, 4);

    -- Ensure loser doesn't go below minimum ($10) - adjust transfer if needed
    IF v_loser_new_cap_4dec < 10.00 THEN
      -- Recalculate transfer to only take what's available above $10 minimum
      v_transfer_amount := ROUND(v_loser_current_cap - 10.00, 4);
      v_loser_new_cap_4dec := 10.00;
      v_winner_new_cap_4dec := ROUND(v_winner_current_cap + v_transfer_amount, 4);
    END IF;
  END IF;

  -- CRITICAL FIX: Round to 2 decimals FIRST, then enforce perfect conservation at 2-decimal precision
  v_winner_new_cap_2dec := ROUND(v_winner_new_cap_4dec, 2);
  v_loser_new_cap_2dec := ROUND(v_loser_new_cap_4dec, 2);
  v_transfer_amount := ROUND(v_transfer_amount, 2);

  -- ENFORCE PERFECT CONSERVATION at 2-decimal precision (this is what gets stored)
  v_pair_total_after_2dec := ROUND(v_winner_new_cap_2dec + v_loser_new_cap_2dec, 2);
  v_conservation_drift_2dec := v_pair_total_before_2dec - v_pair_total_after_2dec;

  -- If there's any drift due to rounding, adjust the winner's cap to eliminate it
  IF ABS(v_conservation_drift_2dec) > 0.001 THEN
    v_winner_new_cap_2dec := ROUND(v_winner_new_cap_2dec + v_conservation_drift_2dec, 2);
    -- Recalculate pair total to verify
    v_pair_total_after_2dec := ROUND(v_winner_new_cap_2dec + v_loser_new_cap_2dec, 2);
    
    -- Final validation: if still not exact, raise exception (this should never happen)
    IF ABS(v_pair_total_after_2dec - v_pair_total_before_2dec) > 0.001 THEN
      RAISE EXCEPTION 'Conservation violation: Before=%, After=%, Drift=%', 
        v_pair_total_before_2dec, v_pair_total_after_2dec, v_conservation_drift_2dec;
    END IF;
  END IF;

  -- Use the 2-decimal values for storage (these maintain perfect conservation)
  v_winner_after_cap := v_winner_new_cap_2dec;
  v_loser_after_cap := v_loser_new_cap_2dec;
  ...
  -- Update teams atomically (only if entries don't exist) - use 2-decimal values
  IF NOT v_home_entry_exists AND NOT v_away_entry_exists THEN
    UPDATE teams SET
      market_cap = v_winner_new_cap_2dec,
      updated_at = NOW()
    WHERE id = v_winner_team_id;

    UPDATE teams SET
      market_cap = v_loser_new_cap_2dec,
      updated_at = NOW()
  ...
  RETURN json_build_object(
    ...
    'pair_total_before', v_pair_total_before_2dec,
    'pair_total_after', v_pair_total_after_2dec,
    'conservation_verified', (ABS(v_pair_total_after_2dec - v_pair_total_before_2dec) < 0.001)
  );
