-- Fetch Match Scores and Process Matches
-- This migration will attempt to fetch scores from Football API and process matches
-- Note: This requires the http extension or external API calls

-- Since we can't directly make HTTP calls from PostgreSQL easily,
-- we'll create a function that can be called with match data
-- For now, we'll process matches that might have scores from previous syncs
...
DECLARE
  ...
  v_fixtures_with_scores INTEGER;
BEGIN
  RAISE NOTICE 'Starting: Fetch and process matches...';
  RAISE NOTICE 'NOTE: This migration processes matches with existing scores.';
  RAISE NOTICE 'To fetch new scores, use the Football API sync service.';

  -- Check if there are any fixtures with scores
  SELECT COUNT(*) INTO v_fixtures_with_scores
  FROM fixtures
  WHERE home_score IS NOT NULL AND away_score IS NOT NULL;

  IF v_fixtures_with_scores = 0 THEN
    RAISE NOTICE '⚠ No fixtures with scores found.';
    RAISE NOTICE 'Skipping processing. Please sync fixtures from Football API first.';
    RETURN;
  END IF;

  RAISE NOTICE 'Found % fixtures with scores. Processing matches...', v_fixtures_with_scores;
  ...
  LOOP
    BEGIN
      ...
      IF v_current_matchday IS NULL OR v_current_matchday != v_fixture.matchday THEN
        ...
        RAISE NOTICE '--- Processing Matchday % (%) ---', v_matchday_count, v_current_matchday;
      END IF;
