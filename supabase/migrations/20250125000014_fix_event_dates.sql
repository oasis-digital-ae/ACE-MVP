-- Fix event_date in total_ledger to use actual fixture dates instead of NOW()
-- This ensures match history shows the correct match dates
...
DECLARE
  ...
  v_winner_shares INTEGER;
  v_loser_shares INTEGER;
...
BEGIN
  ...
  SELECT f.*, f.snapshot_home_cap, f.snapshot_away_cap, f.kickoff_at
  INTO v_fixture
  FROM fixtures f
  ...
  IF v_fixture.snapshot_home_cap IS NOT NULL AND v_fixture.snapshot_away_cap IS NOT NULL THEN
    -- Use snapshot values (preferred for historical accuracy)
    v_home_snapshot_cap := v_fixture.snapshot_home_cap;
    v_away_snapshot_cap := v_fixture.snapshot_away_cap;
  ELSE
    -- Fallback to current market caps (for fixtures without snapshots)
  ...
  ELSE
    ...
    -- Log draw for home team
    SELECT market_cap, shares_outstanding,
           CASE WHEN shares_outstanding > 0 THEN market_cap / shares_outstanding ELSE 20.00 END
    INTO v_home_snapshot_cap, v_loser_shares, v_winner_price_before
    FROM teams WHERE id = v_fixture.home_team_id;
    
    INSERT INTO total_ledger (
      team_id, ledger_type, opponent_team_id, is_home_match, event_date,
      ...
      v_fixture.home_team_id, 'match_draw', v_fixture.away_team_id, TRUE, v_fixture.kickoff_at,
      v_home_snapshot_cap, v_home_snapshot_cap, -- No change
      v_loser_shares, v_loser_shares, -- No change
      v_winner_price_before, v_winner_price_before, 0, -- No price impact
      'draw', v_match_score,
      'fixture', p_fixture_id, 'Draw: No market cap transfer', 'system'
    );
    
    -- Log draw for away team
    SELECT market_cap, shares_outstanding,
           CASE WHEN shares_outstanding > 0 THEN market_cap / shares_outstanding ELSE 20.00 END
    INTO v_away_snapshot_cap, v_loser_shares, v_winner_price_before
    ...
    INSERT INTO total_ledger (
      ...
      v_fixture.away_team_id, 'match_draw', v_fixture.home_team_id, FALSE, v_fixture.kickoff_at,
      v_away_snapshot_cap, v_away_snapshot_cap, -- No change
      v_loser_shares, v_loser_shares, -- No change
      v_winner_price_before, v_winner_price_before, 0, -- No price impact
      'draw', v_match_score,
      'fixture', p_fixture_id, 'Draw: No market cap transfer', 'system'
  ...
  SELECT market_cap, shares_outstanding,
         CASE WHEN shares_outstanding > 0 THEN market_cap / shares_outstanding ELSE 20.00 END
  INTO v_winner_before_cap, v_winner_shares, v_winner_price_before
  FROM teams WHERE id = v_winner_team_id FOR UPDATE;
  
  SELECT market_cap, shares_outstanding,
         CASE WHEN shares_outstanding > 0 THEN market_cap / shares_outstanding ELSE 20.00 END
  INTO v_loser_before_cap, v_loser_shares, v_loser_price_before
  ...
  v_winner_price_after := CASE WHEN v_winner_shares > 0 THEN v_winner_after_cap / v_winner_shares ELSE 20.00 END;
  v_loser_price_after := CASE WHEN v_loser_shares > 0 THEN v_loser_after_cap / v_loser_shares ELSE 20.00 END;
  ...
  UPDATE teams SET
    market_cap = GREATEST(v_loser_after_cap, 10), -- Minimum $10 market cap
    updated_at = NOW()
  ...
  INSERT INTO total_ledger (
    team_id, ledger_type, amount_transferred, opponent_team_id, opponent_team_name, is_home_match, event_date,
    market_cap_before, market_cap_after, shares_outstanding_before, shares_outstanding_after,
    share_price_before, share_price_after, price_impact, match_result, match_score,
    trigger_event_type, trigger_event_id, event_description, created_by
  ) VALUES (
    v_winner_team_id, 'match_win', v_transfer_amount, v_opponent_team_id, v_opponent_team_name, v_is_home_winner, v_fixture.kickoff_at,
    v_winner_before_cap, v_winner_after_cap, v_winner_shares, v_winner_shares,
  ...
  INSERT INTO total_ledger (
    ...
    NOT v_is_home_winner, v_fixture.kickoff_at,
    v_loser_before_cap, GREATEST(v_loser_after_cap, 10), v_loser_shares, v_loser_shares,
