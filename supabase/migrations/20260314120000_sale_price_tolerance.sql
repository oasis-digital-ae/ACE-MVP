-- Relax price validation tolerance for sale (matches purchase tolerance)
-- Frontend fetches price, user may take time to confirm; market cap can shift (e.g. match results).
-- Old tolerance: 10 ten-thousandths ($0.001) - too tight, caused "Price has changed" failures.
-- New tolerance: 100 ten-thousandths ($0.01) - aligns with purchase, allows minor price movement.
...
BEGIN
  ...
  -- Allow 100 ten-thousandths ($0.01) tolerance - minor price shifts (match results, rounding) should not block sale
  IF ABS(v_price_per_share_cents - v_nav_cents_per_share) > 100 THEN
    PERFORM set_config('app.allow_wallet_update', '', true);
